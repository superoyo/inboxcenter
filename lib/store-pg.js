...
async function getSavedReplies(pageId) {
  const { rows } = await getPool().query(
    'SELECT id, text, created_at FROM saved_replies WHERE page_id = $1 ORDER BY created_at',
    [pageId]
  );
  return rows.map((r) => ({ id: r.id, text: r.text, createdAt: r.created_at.toISOString() }));
}

async function addSavedReply(pageId, entry) {
  await getPool().query(
    'INSERT INTO saved_replies (id, page_id, text, created_at) VALUES ($1, $2, $3, $4)',
    [entry.id, pageId, entry.text, entry.createdAt]
  );
  return entry;
}
...
module.exports = {
  ...
  saveConversations, upsertConversations, saveConversation, getAllConversations, getConversationsForPage,
