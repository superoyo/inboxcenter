...
async function getPosts(pageId, accessToken, limit = 25) {
  const json = await graphGet(`/${pageId}/posts`, {
    fields: 'id,message,story,created_time,full_picture,permalink_url,comments.summary(true).limit(0)',
    limit,
    access_token: accessToken,
  });
  return (json.data || []).map((p) => ({
    ...
    commentCount: (p.comments && p.comments.summary && p.comments.summary.total_count) || 0,
  }));
}
...
module.exports = {
  ...
  getPosts, getComments, replyComment,
};
