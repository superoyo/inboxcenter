...
async function getConversations(pageId, accessToken, { limit = 25, messagesPerConversation = 25 } = {}) {
  const conversations = [];
  ...
  let pages = 0;
  while (json.paging && json.paging.next && pages < 10) {
    const res = await fetch(json.paging.next);
    json = await res.json();
    if (json.error) break;
    conversations.push(...(json.data || []));
...
module.exports = { getPageInfo, getUserPages, getConversations, normalizeConversation, fetchProfilePics, sendMessage };
