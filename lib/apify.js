...
// ดึง URL รูปแรกที่หาได้จาก media — actor คืน media เป็น "object" (โพสต์เดียว)
// แต่บางเวอร์ชัน/บางชนิดโพสต์เป็น array จึงรองรับทั้งสองแบบ
const asUrl = (v) => {
...
function imageFromMediaEntry(m) {
  if (!m) return '';
  const direct = asUrl(m);
  if (direct) return direct;
  if (typeof m !== 'object') return '';
  // ไล่ตามเส้นทางที่ actor ใช้จริง (ตรวจจาก output จริงของ apify/facebook-posts-scraper)
  const cands = [
    m.thumbnail, m.thumbnailUrl, m.full_picture,
    m.image, m.thumbnailImage, m.photo_image,
...
function firstImage(p) {
  const direct = asUrl(pick(p, ['thumbnailUrl', 'imageUrl', 'previewImage', 'full_picture', 'image', 'thumbnail']));
  if (direct) return direct;
